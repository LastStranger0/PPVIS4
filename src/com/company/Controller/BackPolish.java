...
public class BackPolish {
    ...
    int runpriority(String operator) {
    ...
    void runCMD(LinkedList<Double> operands, String operator) {
        ...
        if (operator.equals("sin") || operator.equals("cos") || operator.equals("tan") ||
            ...
            switch (operator) {
                case "sin" -> {
                    ...
                    operations = tree.getOperationnames().getLast().replace("sin(" + firstOperand + ")", parent.toString());
                    tree.getOperationnames().add(operations);
                }
                case "cos" -> {
                    ...
                    tree.getRoot().removeAllChildren();
                    tree.getRoot().add(parent);
                    operations = tree.getOperationnames().getLast().replace("cos(" + firstOperand + ")", parent.toString());
                    tree.getOperationnames().add(operations);
                }
                case "tan" -> {
                    ...
                    tree.getRoot().removeAllChildren();
                    tree.getRoot().add(parent);
                    operations = tree.getOperationnames().getLast().replace("tan(" + firstOperand + ")", parent.toString());
                    tree.getOperationnames().add(operations);
                }
                case "sinh" -> {
                    ...
                    tree.getRoot().removeAllChildren();
                    tree.getRoot().add(parent);
                    operations = tree.getOperationnames().getLast().replace("sinh(" + firstOperand + ")", parent.toString());
                    tree.getOperationnames().add(operations);
                }
                case "cosh" -> {
                    ...
                    tree.getRoot().removeAllChildren();
                    tree.getRoot().add(parent);
                    operations = tree.getOperationnames().getLast().replace("cosh(" + firstOperand + ")", parent.toString());
                    tree.getOperationnames().add(operations);
                }
                case "tanh" -> {
                    ...
                    tree.getRoot().removeAllChildren();
                    tree.getRoot().add(parent);
                    operations = tree.getOperationnames().getLast().replace("tanh(" + firstOperand + ")", parent.toString());
                    tree.getOperationnames().add(operations);
                }
            }
        } else {
            ...
            if (!afternode.getUserObject().equals("")) {
                if (firstOperand == Double.valueOf(afternode.toString())) {
                    child1 = afternode;
                    child2 = new DefaultMutableTreeNode(secondOperand);
                    afternode = new DefaultMutableTreeNode();
                } else if (secondOperand == Double.valueOf(afternode.toString())) {
                    child2 = afternode;
                    child1 = new DefaultMutableTreeNode(firstOperand);
                    afternode = new DefaultMutableTreeNode();
                }
            ...
            switch (operator) {
                case "+" -> {
                    ...
                    tree.getRoot().removeAllChildren();
                    tree.getRoot().add(parent);
                    operations = tree.getOperationnames().getLast().replace(secondOperand + "+" + firstOperand, parent.toString());
                    if (operations.equals(tree.getOperationnames().getLast())) {
                        operations = tree.getOperationnames().getLast().replace("(" + secondOperand + ")" + "+" + firstOperand, parent.toString());
                        if (operations.equals(tree.getOperationnames().getLast())) {
                        }
                        else {
        ...
        for (int index = 0; index < string.length(); index++) {
            ...
            if (c.equals("(")) {
                ...
                while (!operators.getLast().equals("(")) {
                    runCMD(operands, operators.removeLast());
                }
                ...
                while (!operators.isEmpty() &&
                        runpriority(operators.getLast()) >= runpriority(c)) {
                    runCMD(operands, operators.removeLast());
                }
                minusChecker = false;
                operators.add(c);
